class DoubleLinkedList:
    ...
    class _Node:
        ...
        __slots__ = '_element', '_next', '_prev'         # streamline memoryusg
    ...
    def add_first(self, e):
        ...
        newest = self._Node(
            e, None, self._head)   # node will be new head node, next point to old head
        if self.is_empty():
    ...
    def add_last(self, e):
        ...
        newest = self._Node(
            e, self._tail, None)            # node will be new tail node, prev point to old tail
        if self.is_empty():
    ...
    def diff(self, otherlist):
        ...
        l3 = DoubleLinkedList()
        while curNode is not None:
            iterateOther = otherlist._head
            while iterateOther is not None:
                if curNode._element == iterateOther._element:
                    break
                elif iterateOther._next is None and iterateOther._element != curNode._element:
                    l3.add_last(curNode._element)
                iterateOther = iterateOther._next
            curNode = curNode._next
        return l3
    ...
    def superFeed(self, otherlist, n):
        ...
        eltsToAdd = []
        for i in range(n):
            eltsToAdd.append(otherlist.delete_first())
        for i in eltsToAdd[::-1]:
            self.add_first(i)
