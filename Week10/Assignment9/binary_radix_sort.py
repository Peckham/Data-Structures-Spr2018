...
def determine_digit(integer, digit):
    ...
    '''
    toBin = bin(integer)[2:]
    if digit > len(toBin) - 1:
        return 0
    else:
        return int(toBin[len(toBin) - digit - 1])
...
def radix_sort(array):
    ...
    '''
    queues = [LinkedQueue(), LinkedQueue()]
    digit = 0
    total_passes = 0
    maximum = 0
    while (digit <= total_passes):
        for each in array:
            if (digit == 0) and (maximum < each):
                maximum = each
            queues[determine_digit(each, digit)].enqueue(each)
        # print(queues)

        if (digit == 0):
            total_passes = int(math.log(maximum, 2))

        index = 0
        for i in range(2):
            while (not queues[i].is_empty()):
                array[index] = queues[i].dequeue()
                index += 1
        digit += 1
