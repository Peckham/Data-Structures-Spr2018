...
class TreeMap(LinkedBinaryTree, MapBase):
    ...
    def iterative_search(self, p, key):
        # Problem 1
        # Owen solved this by creating a stack: append p, if p != k then append
        # Children.
        curNode = p
        while curNode.key() != key:
            if curNode.key() > key:
                if self.left(curNode) is not None:
                    curNode = self.left(curNode)
            elif self.right(curNode) is not None:
                    curNode = self.right(curNode)
            else:
                break
        return curNode
    ...
    def get_kth_largest(self, k):
        # Problem 2
        # Iterate backwards through k values from the last value.
        curPos = self.last()
        for i in range(k):
            curPos = self.before(curPos)
            if curPos is None:
                return None
        return curPos
...
class AVLTreeMap(TreeMap):
    ...
    def _rotate(self, p):
        ...
        if x == y._left:
            ...
            y._balance_factor = y._balance_factor - \
                1 - max(x._balance_factor, 0)
            x._balance_factor = x._balance_factor - \
                1 + min(y._balance_factor, 0)

        else:
            self._relink(y, x._left, False)  # x._left becomes right child of y
            self._relink(x, y, True)  # y becomes left child of x
            y._balance_factor = y._balance_factor + \
                1 - min(x._balance_factor, 0)
            x._balance_factor = x._balance_factor + \
                1 + max(y._balance_factor, 0)
